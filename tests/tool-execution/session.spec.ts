...
import { createSession, createSessionWithBranch, expandToolOutput, getMessageByContent, getSessionIdFromUrl, navigateToSessions, openNewSessionDialog } from "../pages/sessions";
...
test.describe('Tool Execution Tests', () => {
  ...
  test('safeBash tool execution to get commit SHA', async ({ page, trackCurrentSession }) => {
    ...
    await expect(getMessageByContent(page, /\b[a-f0-9]{40}\b/i).last()).toBeVisible({ timeout: 120000 });
  });
});