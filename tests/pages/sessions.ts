...
/**
 * Returns the initial-prompt textbox in the "Create new session" dialog.
 *
 * This textbox is referenced by many tests (drag-and-drop uploads, paste, typing
 * the initial prompt), so centralize its locator here instead of repeating the
 * placeholder string across spec files.
 *
 * @param page The Playwright page object
 * @returns A locator for the initial-prompt textbox
 */
export function getInitialPromptInput(page: Page): Locator {
...
export async function submitNewSessionDialog(page: Page, prompt: string): Promise<void> {
  await getInitialPromptInput(page).fill(prompt);
