...
/**
 * Returns the chat message input textbox locator for the active session.
 *
 * The message composer is used all over the session suite (send/steer/queue,
 * enabled/visible assertions, clipboard paste). Centralizing the locator here
 * keeps every call site in sync if the accessible name/role ever changes.
 *
 * Assumes the page is already on a session detail page with the chat interface visible.
 *
 * @param page The Playwright page object
 * @returns The message input textbox locator
 */
