...
test.describe('Issues Tests', () => {
  ...
  test('fetch video analysis tool in triage session', async ({ page, trackCurrentSession }) => {
    ...
    const analyseVideoLocator = page.getByText("Used analyseVideo").or(page.getByText("Used analyse_video")).first();
    await expect(analyseVideoLocator).toBeVisible({ timeout: 180000 });
    
    // Click on the tool execution result to see the analysis
    await analyseVideoLocator.click();
