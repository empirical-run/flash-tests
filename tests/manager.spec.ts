...
test.describe('Manager Page', () => {
  test('opens the live project manager with its lineage and message composer', async ({ page }) => {
    await navigateToManager(page);

    const projectSlug = getProjectSlug();
    await expect(page).toHaveURL(new RegExp(`/${projectSlug}/manager\\?session=\\d+$`));
    await expect(page.getByText('Manager Lineage', { exact: true })).toBeVisible();
    ...
    const sessionCount = Number((await sessionCountLabel.textContent())?.match(/\d+/)?.[0]);
    expect(sessionCount).toBeGreaterThan(0);

    const managerLinks = page.getByRole('link', { name: /^Manager #\d+/ });
    await expect(managerLinks).toHaveCount(sessionCount);

    const liveManagerLink = page.getByRole('link', { name: /^Manager #\d+ Live/ });
    ...
    await expect(lineageDialog.getByText('Manager sessions and executors')).toBeVisible();
    await expect(lineageDialog.getByRole('link', {
