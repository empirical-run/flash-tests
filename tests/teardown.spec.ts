...
test("Teardown: Close all automation test sessions", async ({
  ...
  await loggedInPage
    ...
    .click();
  await expect(loggedInPage.getByRole('tab', { name: 'Active Sessions' })).toBeVisible();
  await loggedInPage.waitForTimeout(10_000);
  const sessionRow = loggedInPage.getByRole('row', { name: 'automation-test@empirical.run' }).first()
