...
test.describe('Mobile Session Tests', () => {
  test('create new session and send chat message', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Type your message...' }).fill(chatMessage);
    await page.locator('div').filter({ hasText: new RegExp(`^${chatMessage}import { test, expect } from '../fixtures';
