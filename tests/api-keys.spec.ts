...
test.describe("API Keys", () => {
  ...
  test("verify initial status of new API key is 'Enabled'", async ({ page }) => {
    ...
    for (const pattern of allPatterns) {
      ...
      const rows = page.getByRole('row').filter({ hasText: new RegExp(pattern.replace(/[.*+?^${}()|[\]\\]/g, '\\    console.log('✅ Test API key cleaned up successfully');
  });
});')) });
      const count = await rows.count();
      
      if (count > 0) {
        console.log(`Found ${count} API keys matching pattern: "${pattern}"`);
        
        // Delete each matching API key
        for (let i = 0; i < count; i++) {
          try {
            // Get the first matching row (index 0 because rows get removed after deletion)
