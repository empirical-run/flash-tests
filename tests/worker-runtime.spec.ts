...
test.describe('Worker Runtime', () => {
  // FIXME: The "worker" chat-session mode is not deployed yet. There is no UI
  // control to set it, so this test injects `mode: "worker"` into the
  // POST /api/chat-sessions request via route interception. Unskip this test
  // (remove the test.fixme below) once the worker runtime ships to production.
  test.fixme('creates a worker-mode session and gets a tool-backed time reply', async ({ page, trackCurrentSession }) => {
    ...
    await expect(getChatMessageByText(page, firstMessage)).toBeVisible({ timeout: 30000 });
