...
import {
  appendPromptAndCreateUploadedFileSession,
  dragAndDropFile,
  pasteFile,
} from "./pages/upload";
...
const LOOK_AT_AGENT_TOOL_REGEX =
  /(?:Used look-at-agent tool|Look At Agent completed)/i;

test.describe("Session file uploads", () => {
  test("upload file via drag and drop during session creation", async ({
    page,
    trackCurrentSession,
  }) => {
    ...
    const textarea = page.getByPlaceholder(
      "Enter an initial prompt or drag and drop a file here",
    );
    ...
    await appendPromptAndCreateUploadedFileSession(
      page,
      textarea,
      SESSION_PROMPT,
    );
    ...
    test
      .info()
      .annotations.push({ type: "Session URL", description: page.url() });

    // Verify the user message bubble with the upload URL loads after session is created
    await expect(
      page.locator("[data-message-id]").filter({ hasText: UPLOAD_URL_REGEX }),
    ...
    await expect(
      page
        .getByRole("link", { name: UPLOAD_URL_REGEX })
        .and(page.locator('[target="_blank"]')),
    ).toBeVisible({ timeout: 15000 });

    // Verify the assistant uses the image-reading tool to process the uploaded file
    await expect(page.getByText(LOOK_AT_AGENT_TOOL_REGEX)).toBeVisible({
      timeout: 120000,
    });

    // Verify the assistant reads the correct download speed from the image
    await expect(page.getByText("8.80 Mbps").first()).toBeVisible({
      timeout: 30000,
    });
  });

  test("upload file via paste during session creation", async ({
    page,
    trackCurrentSession,
  }) => {
    await navigateToSessions(page);
