...
import { getChatMessage, navigateToSessions, openNewSessionDialog } from "./pages/sessions";
...
test.describe('Session file uploads', () => {
  test('upload file via drag and drop during session creation', async ({ page, trackCurrentSession }) => {
    ...
    await expect(getChatMessage(page, UPLOAD_URL_REGEX).first()).toBeVisible({ timeout: 30000 });
