...
test.describe("Magic Link Login", () => {
  ...
  test("shows appropriate message when unregistered user clicks magic link", async ({ page }) => {
    ...
    // For unregistered users, the magic link now redirects to the login page 
    // with the token included in the URL parameters
    await expect(page).toHaveURL(/.*\/login.*token_hash=/);
    
    // Verify the login page loads correctly with available login options
