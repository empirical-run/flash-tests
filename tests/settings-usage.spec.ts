...
const USAGE_ROWS = [
  ...
  { name: 'Manager initiated sessions', description: 'Sessions started by the manager' },
  { name: 'User initiated sessions', description: 'Sessions started directly by users' },
...
test.describe('Usage Settings Page', () => {
  test('shows categorized AI usage totals and switches the reporting month', async ({ page }) => {
    await navigateToUsage(page);

    await expect(page.getByRole('heading', { name: 'Usage', level: 1 })).toBeVisible();
    await expect(page.getByText('See monthly AI and run infrastructure usage for this project.')).toBeVisible();
    await expect(page.getByRole('heading', { name: 'AI usage', level: 2 })).toBeVisible();
    await expect(page.getByText('Sessions, tokens, and credits used by AI activity.')).toBeVisible();

    const now = new Date();
    const monthPicker = page.getByRole('combobox', { name: 'Usage month' });
    await expect(monthPicker).toContainText(`${monthLabel(now)} (month to date)`);
    await expect(page).toHaveURL(new RegExp(`[?&]month=${monthValue(now)}(?:&|$)`));
    ...
    for (const usage of USAGE_ROWS) {
      const row = usageTable.getByRole('row').filter({ hasText: usage.description });
      await expect(row.getByText(usage.name, { exact: true })).toBeVisible();
      await expect(row.getByText(usage.description, { exact: true })).toBeVisible();
      const values = await readNumericCells(row);
      expect(values.every(Number.isFinite)).toBe(true);
      categoryValues.push(values);
    }

    const totalRow = usageTable.getByRole('row').filter({ has: page.getByRole('cell', { name: 'Total' }) });
    ...
    await page.getByRole('option', { name: monthLabel(previousMonth), exact: true }).click();

    await expect(page).toHaveURL(new RegExp(`[?&]month=${monthValue(previousMonth)}(?:&|$)`));
