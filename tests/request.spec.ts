...
test("should be able to create new request and verify a new chat session is created and title and description from the request are visible in the chat session", async ({ page }) => {
  ...
  // Open the session by clicking on the specific session row for our request using title attribute
...
test("should preserve request description when canceling edit", async ({ page }) => {
  ...
  // TODO(agent on page): Debug by inspecting elements in the requests table to find cells with title attributes and then click on the correct cell for our request title
